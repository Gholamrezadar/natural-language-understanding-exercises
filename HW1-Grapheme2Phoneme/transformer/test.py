...
def validate(model, dataloader, show=10):
    ...
    with torch.no_grad():
        for batch in dataloader:
            ...
            if show > show_count:
                show_count += 1
                print(f"{''.join(real_g)} -> {''.join(real_p)} -> {''.join(out_p)}")

            elif show_count == show:
...
if __name__ == "__main__":
    ...
    dataset = TextLoader(path="test_data.txt")
    ...
    model.load_state_dict(torch.load("g2p_model_transformer_best.pt"))

    # Test
    max_len = 50
    test_loader = torch.utils.data.DataLoader(dataset, num_workers=2, shuffle=False,
                                            batch_size=1, pin_memory=False,
                                            drop_last=False, collate_fn=collate_fn)

    cer, wer = validate(model, test_loader, show=10)
    print("cer", cer)
    print("wer", wer)